...
class YOLODetectorQueue:
    ...
    # def detect_for_one_frame(self, data: Union[str, np.ndarray]):
    #     frame_idx = 0
    #     # expected frame shape like (1,3,h,w) or (3,h,w)
    #     img_k, orig_img, im_dim_list_k = self.detector.image_preprocess(
    #         data
    ...
    def detect_for_one_frame(self, data: Union[str, np.ndarray]):
        ...
        with torch.no_grad():  # Record original image resolution
            im_dim_list_k = torch.FloatTensor(im_dim_list_k).repeat(1, 2)
            img_det = self.image_detection((img_k, orig_img, im_name, im_dim_list_k))   # model work
            item = self.image_postprocess(img_det)
            return item  ,self.detector.detect_fps
    ...
    def tracker_udpate(self, dets, orig_img):
        ...
        for t in online_targets:
            if t.tlwh[2] * t.tlwh[3] > self.min_box_area:
