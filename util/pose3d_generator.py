...
from Pose3D.dataset_wild import PoseDataset, resample_data
...
class Pose3DGenerator:
    ...
    def __init__(
        ...
        self.cfg = cfg
        self.pixel = self.cfg.pixel
        self.clip_len = self.cfg.clip_len
        self.focus = self.cfg.focus
        self.no_conf = self.cfg.no_conf
        self.flip = self.cfg.flip
        self.rootrel = self.cfg.rootrel
        self.gt_2d = self.cfg.gt_2d
        self.results_all = []
        self.pose_fps = 0
    ...
    def load_model(self):
        ...
        checkpoint = torch.load(
            self.cfg.evaluate, map_location=lambda storage, loc: storage
        )
        model_backbone.load_state_dict(checkpoint["model_pos"], strict=True)
        self.model = model_backbone
        self.model.eval()

    def resmaple_datas(self, kpts, is_pixel=False):
        self.is_pixel = is_pixel
        vid_size = None if not is_pixel else self.video_info["frameSize"]
        scale_range = None if is_pixel else [1, 1]
        wild_dataset = resample_data(kpts, vid_size, scale_range)
        return wild_dataset

    def predict_3D_Pose(self, kpts):
        kpts=np.expand_dims(kpts, axis=0)
        wild_dataset = torch.tensor(kpts)
        wild_dataset = wild_dataset.to(torch.float32)
        pose3d = self.uplift3D(wild_dataset)
        pose3d = pose3d.cpu().numpy()
        if self.is_pixel:
            # Convert to pixel coordinates
            pose3d[0] = pose3d[0] * (min(self.video_info["frameSize"]) / 2.0)
            pose3d[0, :, :, :2] = (
                pose3d[0, :, :, :2] + np.array(self.video_info["frameSize"]) / 2.0
            )
        return pose3d  # [1,243,17,3]
    ...
    def preprocess_skeletons(self, file_path, file_basename, out_path="."):
        ...
        kpts_clips = {clip_id: []}  # motion dict
        ...
        for _id, kpts in kpts_clips.items():
            ...
            self.save_3D_Pose(
                rf"{out_path}\{basename}", f"{basename}_{_id}", results_all
            )

    def uplift3D(self, kpts):
        ...
        with torch.no_grad():
            ...
            if self.no_conf:
                kpts = kpts[:, :, :, :2]
            if self.flip:
                kpts_flip = flip_data(kpts)
                predicted_3d_pos_1 = self.model(kpts)
                predicted_3d_pos_flip = self.model(kpts_flip)
                predicted_3d_pos_2 = flip_data(predicted_3d_pos_flip)  # Flip back
                predicted_3d_pos = (predicted_3d_pos_1 + predicted_3d_pos_2) / 2.0
            else:
                predicted_3d_pos = self.model(kpts)
            if self.rootrel:
    ...
    def save_3D_Pose(self, folder, basename, results_all, for_eval=True):
        ...
        self.render_3D_plot(
            results_all, rf"{folder}/{basename}.mp4", keep_imgs=False, fps=fps
        )
        ...
        if self.pixel:
            ...
            results_all[:, :, :2] = (
                results_all[:, :, :2] + np.array(self.video_info["frameSize"]) / 2.0
            )
    ...
    def render_3D_plot(
        ...
        self.motion2video_3d(
            motion_world, save_path=save_path, keep_imgs=keep_imgs, fps=fps
        )
    ...
    def motion2video_3d(self, motion, save_path, fps=60, keep_imgs=False):
        ...
        writer = cv2.VideoWriter(save_path, fourcc, fps, (500, 500))
