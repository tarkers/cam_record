...
from Pose3D.lib.utils.utils_data import crop_scale


def perpare_id_json(results, folderpath="./"):
    focus_ids = {}
    for item in results:
        new_data = {
    ...
    # save json by id
    for id in focus_ids:
        with open(rf"{folderpath}\{id}.json", "w") as json_file:
...
class PoseDataset(Dataset):
    def __init__(
        self, key_points, clip_len=243, vid_size=None, scale_range=None, focus=None
    ):
        self.clip_len = clip_len
...
def resample_data(key_points, vid_size, scale_range):
    key_points = halpe2h36m(key_points)
    if vid_size:
        w, h = vid_size
        scale = min(w, h) / 2.0
        key_points[:, :, :2] = key_points[:, :, :2] - np.array([w, h]) / 2.0
        key_points[:, :, :2] = key_points[:, :, :2] / scale
        motion = key_points
        # print(motion.shape,"Test vid_size")
    if scale_range:
        motion = crop_scale(key_points, scale_range)
