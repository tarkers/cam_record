...
from pose2D.utils.transforms import get_func_heatmap_to_coord
from pose2D.utils.pPose_nms import pose_nms, write_json
    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                print(f"Results json have been written to {self.opt.outputpath}.")
                return
            ...
            if boxes is None or len(boxes) == 0:
                ...
                if self.opt.save_img or self.save_video or self.opt.vis:
                    if hm_data.size()[1] == 49:
                        from pose2D.utils.vis import vis_frame_dense as vis_frame
                    elif self.opt.vis_fast:
                        from pose2D.utils.vis import vis_frame_fast as vis_frame
                    else:
                        from pose2D.utils.vis import vis_frame
                    img = vis_frame(orig_img, result, self.opt, self.vis_thres)
                    self.write_image(img, im_name, stream=stream if self.save_video else None)

    def write_image(self, img, im_name, stream=None):
        if self.opt.vis:
            cv2.imshow("pose2D.Demo", img)
            cv2.waitKey(30)
