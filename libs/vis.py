...
def read_gt(filename):
    ...
    try:
        ...
        file = pd.DataFrame(df[[("RKneeAngles", "X"), ("LKneeAngles", "X")]]).copy()
...
def vis_angle(basename, data, out_folder="out_mask", show=True, linewidth=2):
    ...
    for i in range(data_nums):
        ...
        color = data["color"][i]
        plt.plot(
            left, linestyle="solid", color=color, linewidth=linewidth, label=f"{name}_L"
        )
...
def plot_2d_skeleton(
    frame, im_res, showbox=False, tracking=False, fast=True, format="coco"
):
    """
    ...
    im_result = im_res["result"]
    if len(im_result) > 0:
        # if fast:
        #     kp_num=17
        # else:
        #     kp_num = len(im_res['result'][0]['keypoints'])
        kp_num = len(im_result[0]["keypoints"])
        
    if kp_num == 17:
        if format == "coco":
            line_color = COCO["line_color"]
            p_color = COCO["p_color"]
            l_pair = COCO["l_pair"]
    else:
        line_color = HALPHE["line_color"]
        p_color = HALPHE["p_color"]
        l_pair = HALPHE["l_pair"]

    img = frame.copy()
    for human in im_result:
...
def plot_single_2d_skeleton(
    ...
    for n in range(len(vis_thres)):
        ...
        if n < len(p_color):
            ...
            cv2.circle(img, (cor_x, cor_y), 1, (255, 255, 255), 3)
    ...
    for i, test in enumerate(tests):
        ...
        tmp = np.array(
            [
                ...
                cal_MAE(tr, tgr, 1),
                cal_MAE(tl, tgl, 1),
                cal_SD(tr, tgr, r_mean, 1),
                cal_SD(tl, tgl, l_mean, 1),
                gradient_analysis(tr, tgr, 1),
                gradient_analysis(tl, tgl, 1),
            ]
        )
