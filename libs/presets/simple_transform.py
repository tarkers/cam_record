    ...
    def __call__(self, src, label):
        ...
        if self._loss_type == "MSELoss":
            ...
            if self.num_joints == 68:
                ...
                hand_face_num = 110
            target_mse, target_weight_mse = self._target_generator(
                joints[:-hand_face_num, :, :], self.num_joints - hand_face_num
            )
            target_inter, target_weight_inter = self._integral_target_generator(
                joints[-hand_face_num:, :, :], hand_face_num, inp_h, inp_w
            )
        ...
        if self._loss_type == "Combined":
            return (
                img,
                [torch.from_numpy(target_mse), torch.from_numpy(target_inter)],
                [
                    torch.from_numpy(target_weight_mse),
                    torch.from_numpy(target_weight_inter),
                ],
                torch.Tensor(bbox),
            )
        else:
