from Ui_uplift import Ui_MainWindow
...
class Ui(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.cfg=update_config(r"libs\configs\configs.yaml")
    ...
    def create_model(self):
        ...
        self.tracker = BoTSORT(self.cfg.TRACKER, frame_rate=100.0)
        
        # create detector configuration
        det_cfg=self.cfg.Detector+self.cfg.Common
        det_cfg.debatch=det_cfg.detbatch * len(det_cfg.gpus)
        self.detector= webCamDetectQueue(self.video, det_cfg, get_detector(det_cfg),self.tracker)
        self.video_info=self.detector.videoinfo
        self.video_info['name']=os.path.basename(self.video)
        
        # create pose queue
        pose_cfg = self.cfg.Pose2D
        pose_cfg.min_box_area=0
        self.pose_queue=Pose2DQueue(pose_cfg)
        
        # create writer queue
        writer_cfg = self.cfg.Writer
        self.writer=WriterDQueue(writer_cfg,self.video_info,"Test",True)
